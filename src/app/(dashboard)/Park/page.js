...
import parks from './data'; // Change data source name if needed
  ...
  return (
    <div className="font-['Public Sans'] w-full space-y-6">
      ...
      <div className="flex justify-end mb-4">
        <button
          onClick={() => router.push('/Park/AddNew')}
        ...
        >
          ...
          <span className="text-sm">Add Park</span>
        </button>
      </div>

      {/* Header */}
      <div className="bg-white rounded-t-md px-4 py-3 sm:px-6 sm:py-4 shadow-sm">
        <h1 className="text-lg font-medium text-[#2F2B3DE5]">Park Listing</h1>
      </div>
        ...
        <table className="min-w-full bg-white">
          <thead>
            <tr className="text-left text-sm text-[#2F2B3D99] border-b border-gray-300 h-14">
              ...
              <th className="p-4">INSTALLATIONS</th>
          ...
          <tbody>
            {parks.map((park, index) => (
              <tr key={index} className="text-[#2F2B3D8C] border-b border-gray-300">
                <td className="p-4">{park.id}</td>
                <td className="p-4">{park.location}</td>
                <td className="p-4">
                  <div className="flex items-center gap-4">
                    <div className="w-[95px] h-[52px] border border-gray-300 rounded-md overflow-hidden">
                      <Image src={park.image} alt="Park" width={95} height={52} className="object-cover" />
                    </div>
                    <div>
                      <span className="text-sm font-medium text-[#2F2B3D]">{park.name}</span>
                      <div
                ...
                <td className="p-4">
                  <button
                    onClick={() => router.push('/Park/ParkInfo')}
          ...
          <div key={index} className="bg-white rounded-lg shadow-md p-4 space-y-3">
            <div className="flex items-center gap-3">
              <div className="w-[80px] h-[50px] border rounded overflow-hidden">
                <Image src={park.image} alt="Park" width={80} height={50} className="object-cover" />
              </div>
              <div>
                <div className="text-sm font-medium text-[#2F2B3D]">{park.name}</div>
                <div
