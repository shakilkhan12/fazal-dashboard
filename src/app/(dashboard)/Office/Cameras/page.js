...
const CameraPage = () => {
  ...
  const cameraData = [
    { name: 'Frontdoor camera (CAM-001)', status: 'active', date: '26 July 2025', addedDate: '12 July 2025', addedTime: '10:25PM' },
    { name: 'Lobby camera (CAM-002)', status: 'inactive', date: '25 July 2025', addedDate: '11 July 2025', addedTime: '3:15PM' },
    { name: 'Parking Lot camera (CAM-003)', status: 'inactive', date: '25 July 2025', addedDate: '10 July 2025', addedTime: '8:00AM' },
    { name: 'Reception camera (CAM-004)', status: 'active', date: '26 July 2025', addedDate: '12 July 2025', addedTime: '2:45PM' },
  ];
    ...
    <MunicipalityOfficePage>
      <div className="max-w-6xl mx-auto py-6 px-4 sm:px-6 space-y-6 w-full">
        ...
        <div className="w-full bg-white rounded-md shadow">
          {/* Table header */}
          <div className="hidden md:grid grid-cols-4 text-[13px] font-semibold text-gray-700 bg-gray-100 py-3 px-4">
            <div>NAME</div>
            <div>STATUS</div>
            <div>DATE ADDED</div>
            <div>ACTION</div>
          ...
          <div className="flex flex-col divide-y divide-gray-200 md:divide-none">
            {cameraData.map((cam, idx) => (
              <div key={idx} className="flex flex-col md:grid md:grid-cols-4 md:items-center px-4 py-4 md:py-3 gap-3 md:gap-0">
                {/* NAME */}
                <div className="flex items-center gap-2">
                  <Image src="/Office/device-cctv.png" alt="Camera" width={16} height={16} />
                  <span className="text-[13px] font-sans truncate">{cam.name}</span>
                </div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-[13px] text-[#444050B2]">{cam.date}</span>
                  </div>
                </div>

                {/* DATE ADDED (Desktop) */}
                <div className="hidden md:flex md:flex-col">
                  <span className="text-[13px]">{cam.addedDate}</span>
                  <span className="text-[13px] text-gray-400">{cam.addedTime}</span>
                </div>
                ...
                <div className="flex flex-col mt-2 text-[13px] text-gray-500 md:hidden">
                  <span>{cam.addedDate}</span>
                  <span>{cam.addedTime}</span>
                </div>
              </div>
            ))}
          </div>
