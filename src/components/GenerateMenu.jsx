...
export const GenerateVerticalMenu = ({ menuData }) => {
  ...
  const renderMenuItems = data => {
    ...
    return data.map((item, index) => {
      ...
      const { label, icon, prefix, suffix, arrow, ...rest } = menuItem

      // Localize the href
      const href = rest.href
      const Icon = icon ? <i className={icon} /> : null
      const Arrow = arrow ? <i className={arrow} /> : null
      const menuItemPrefix = prefix && prefix.label ? <CustomChip size='small' round='true' {...prefix} /> : prefix
      const menuItemSuffix = suffix && suffix.label ? <CustomChip size='small' round='true' {...suffix} /> : (Arrow || suffix)
